...
const Header: React.FC = () => {
  ...
  return (
    <>
      ...
      <header className={`fixed top-0 right-0 w-full z-50 transition-[background-color,border-color,box-shadow] duration-300 ${
        isScrolled || isMenuOpen
          ? 'bg-white dark:bg-[#14202f] border-b border-[#14202f]/15 dark:border-[#f5f1e8]/15 shadow-[0_8px_24px_rgba(20,32,47,0.08)]'
          : 'bg-transparent bg-[#14202f] border-b border-[#f5f1e8]/10'
      }`}>
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16 sm:h-20">
          ...
          {/* Desktop Navigation */}
          <nav className={`hidden lg:flex items-center gap-1 rounded-full border px-2 py-1 ${isScrolled ? 'border-[#14202f]/10 bg-[#14202f]/[0.04] dark:border-[#f5f1e8]/15 dark:bg-[#f5f1e8]/[0.05]' : 'border-[#f5f1e8]/15 bg-[#f5f1e8]/[0.06]'}`}>
            {navItems.map((item) => (
              <button
                key={item.label}
                onClick={() => handleNavClick(item)}
                className={`group mono-font text-[11px] uppercase tracking-[0.12em] relative px-3 py-2 rounded-full font-medium transition-colors duration-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#d8f52b] ${isScrolled ? 'text-[#14202f] hover:bg-[#14202f] hover:text-[#f5f1e8] dark:text-[#f5f1e8] dark:hover:bg-[#f5f1e8] dark:hover:text-[#14202f]' : 'text-[#f5f1e8] hover:bg-[#d8f52b] hover:text-[#14202f]'}`}
              >
                {item.label}
                <span className="absolute bottom-1 left-1/2 h-px w-0 -translate-x-1/2 bg-current transition-all duration-200 group-hover:w-1/2" aria-hidden="true" />
              </button>
            ))}
          </nav>
          ...
          <div className="hidden lg:flex items-center space-x-3">
            ...
            <div className="w-px h-6 bg-[#14202f]/20 dark:bg-[#f5f1e8]/20"></div>
             <Button asChild
               variant="outline"
               className={`rounded-none px-4 py-2 border-[#d8f52b] bg-transparent hover:bg-[#d8f52b] hover:text-[#14202f] ${isScrolled ? 'text-[#14202f] dark:text-[#d8f52b]' : 'text-[#d8f52b]'}`}
             >
               ...
               <div className="flex justify-center space-x-4 pt-4 mt-2 border-t border-[#f5f1e8]/15">
                {[
                  ...
                  <Button 
                    ...
                    size="icon" 
                    asChild 
                    className="text-foreground w-12 h-12 rounded-full hover:bg-secondary border border-transparent hover:border-border transition-colors"
                  >
                    <a 
                      href={social.href} 
                      target="_blank" 
